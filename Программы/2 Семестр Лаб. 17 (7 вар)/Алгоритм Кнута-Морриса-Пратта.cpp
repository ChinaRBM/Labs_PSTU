...
int KnutSearch(string& str, string& pat)
{
	...
	strsize = str.size();
	patsize = pat.size();
	if (strsize == 0) cout << "Строка пустая!" << endl;
	else if (patsize == 0) cout << "Образ пуст!" << endl;
	else
	{
		int i = 0, j = -1;
		int* d = new int[100];
...
int main()
{
	...
	if (KnutSearch(str, pat) != -1) cout << "Индекс первого символа подстроки в строке: " << index << endl;
